  ...
  <div class="detail">
            ...
            <div class="chooseArea">
              ...
              <dl v-for="(spu) in spuSaleAttrList" :key="spu.id">
                ...
                <dd changepirce="0" 
                 ...
                 v-for="(spu2) in spu.spuSaleAttrValueList"
                 :key="spu2.id"
                 @click="changeActive(spu2,spu.spuSaleAttrValueList)"
                 >{{spu2.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="num" @change="changeNum">
                <a href="javascript:" class="plus" @click="plus(1)">+</a>
                <a href="javascript:" class="mins" @click="mins(1)">-</a>
              </div>
              <div class="add">
    ...
    <section class="product-detail">
      ...
      <div class="detail">
        ...
        <div class="intro">
          <ul class="tab-wraped" >
            <li v-for="(tab,index) in tabs" :key="index" @click="tabchange(index)" :class="{active:index==currentIndex}"> 
              <a >
                {{tab.title}}
              </a>
            </li>
...
<script>
  ...
  export default {
    ...
    computed:{
     ...
     //如果skuInfo的数据还没有回来 skuInfo.skuImageList就会是undefined 遍历skuImageList就会报错
     skuImageList(){
    ...
    methods:{
      ...
      async addShopCart(){
        try {
            ...
            sessionStorage.setItem("SKUINFO",JSON.stringify(this.skuInfo));//商品对象存到会话存储中，这样在购物车页面中，直接去找浏览器的会话存储要数据就行了
        } catch (error) {
          alert(error)
        }
      }
